...
return this.each(function(){var $this=$(this);var data=$this.data('typeahead');var options=typeof option=='object'&&option;if(!data)$this.data('typeahead',(data=new Typeahead(this,options)));if(typeof option=='string'&&data[option]){if(arg.length>1){data[option].apply(data,Array.prototype.slice.call(arg,1));}else{data[option]();}}});};$.fn.typeahead.defaults={source:[],items:8,menu:'<ul class="typeahead dropdown-menu" role="listbox"></ul>',item:'<li><a class="dropdown-item" href="#" role="option"></a></li>',minLength:1,scrollHeight:0,autoSelect:true,afterSelect:$.noop,addItem:false,delay:0,separator:'category',headerHtml:'<li class="dropdown-header"></li>',headerDivider:'<li class="divider" role="separator"></li>'};$.fn.typeahead.Constructor=Typeahead;$.fn.typeahead.noConflict=function(){$.fn.typeahead=old;return this;};$(document).on('focus.typeahead.data-api','[data-provide="typeahead"]',function(e){var $this=$(this);if($this.data('typeahead'))return;$this.typeahead($this.data());});}));;(function($){"use strict"
...
callback(e)}})}}(jQuery));var contact=function(e){var s=$(e);var p=s.data('parent')?$(s.data('parent')):$('#contact-form');var f=p.find('form');var b=p.find('.btn-submit');var r=p.find('[name=url-request]');var c=p.find('[name=url-complete]');b.prop('disabled',true);$('.loading').fadeIn({duration:300});$.ajax({url:r.val(),type:'post',dataType:'json',data:f.serialize(),success:function(reponsve){if(reponsve!=200){alert(reponsve.message);}
else{document.location.href=c.val();}},error:function(xhr,status){console.log(xhr.status);},complete:function(){$('.loading').fadeOut({duration:300});b.prop('disabled',false);}});return false;};var load_cart_content=function(){$.ajax({url:base_url+'gio-hang-2',type:'GET',data:{},success:function(data){$('#cart-content').html(data);},error:function(xhr,status){console.log(xhr,status);},complete:function(){}});};var add_cart=function(e){var p=$(e).data('wrapper')?$($(e).data('wrapper')):'#cart';var form=p.find('form');var red=$(e).data('redirect');var ure=p.find('[name=url-request]');var uco=p.find('[name=url-complete]');var btn=p.find('.btn-add-cart');btn.prop('disabled',true);$('.loading').fadeIn({duration:300});$.ajax({url:ure.val(),type:'post',dataType:'json',data:form.serialize(),success:function(response){if(response.code!=200){alert(response.message);}
else{if(red==true){document.location.href=uco.val();}
else{if($('.cart-badge').length){$('.cart-badge').text(response.data.total_item);}
$('.cart-notify').fadeIn();setTimeout(function(){$('.cart-notify').fadeOut();},5000);}}},error:function(xhr,status){console.log(xhr.status);},complete:function(){btn.prop('disabled',false);$('.loading').fadeOut({duration:300});}});};var update_cart=function(e){var self=$(e);var rowid=self.attr('data-rowid');$('.loading').fadeIn({duration:300});$.ajax({url:base_url+'cap-nhat-gio-hang',type:'POST',data:{rowid:rowid,qty:self.val()},success:function(data){if(data=='SUCCESS'){load_cart_content();}
else{alert(data);}},error:function(xhr,status){console.log(xhr.status);},complete:function(){$('.loading').fadeOut({duration:300});}});};var remove_cart=function(e){var self=$(e);var rowid=self.attr('data-rowid');$('.loading').fadeIn({duration:300});$.ajax({url:base_url+'xoa-gio-hang',type:'POST',data:{rowid:rowid},success:function(data){if(data=='SUCCESS'){load_cart_content();}
else if(data=='EMPTY'){location.reload();}
else{alert(data);}},error:function(xhr,status){console.log(xhr.status);},complete:function(){$('.loading').fadeOut({duration:300});}});};var payment=function(){var p=$('#payment');var f=p.find('form');var u=p.find('[name=url-request]');var c=p.find('[name=url-complete]');var b=p.find('.btn-submit');b.prop('disabled',true);$('.loading').fadeIn();$.ajax({url:u.val(),type:'post',dataType:'json',data:f.serialize(),success:function(response){if(response.code!=200){alert(response.message);}
else{document.location.href=c.val();}},complete:function(){b.prop('disabled',false);$('.loading').fadeOut();}});};$(document).ready(function(){if($('.lazy').length){$('.lazy').lazy({scrollDirection:'vertical',effect:'fadeIn'});}
if($('.input-spinner').length){$('.input-spinner').inputSpinner({'groupClass':'input-group-custom'});}
if($('[data-toggle=scroll]').length){$('[data-toggle=scroll]').click(function(){var self=$(this);var target=$(self.attr('data-target'));$('html, body').animate({scrollTop:target.offset().top},500);});}
...
$('[data-toggle="tooltip"],[data-provide="tooltip"]').tooltip();$('.homepage-slider .owl-carousel').owlCarousel({autoplay:true,loop:true,items:1,margin:1,nav:true,dots:true,autoplayTimeout:4500,autoplayHoverPause:true,navText:['<i class="icofont-simple-left"></i>','<i class="icofont-simple-right"></i>']});$('.owl-dot').each(function(){$(this).attr('aria-label','Item dot');});if($('.cart-notify').length){$('.cart-notify').find('.remove').on('click',function(){$('.cart-notify').hide();});}});